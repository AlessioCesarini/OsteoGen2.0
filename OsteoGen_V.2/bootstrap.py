...
Dependency check that runs before anything else in pipeline.py. Uses only
the standard library (no third-party imports here), so it works even on a
bare Python install: if a required package is missing, it installs it with
pip for the interpreter currently running the script, instead of the run
dying on an import error partway through.
"""
