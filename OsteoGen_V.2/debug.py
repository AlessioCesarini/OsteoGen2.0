...
def salva_heatmap_inferenza(image_path, model_path, output_path, img_size=512):
    ...
    # Extract the raw heatmaps (no threshold or coordinate conversion)
    with torch.no_grad():
        output = model(img_tensor)
        heatmaps = output.squeeze(0).cpu().numpy()

    # Denormalize the tensor for correct color display
    img_vis = img_tensor.squeeze(0).cpu().numpy().transpose(1, 2, 0)
    img_vis = (img_vis * 0.5) + 0.5

    # Collapse the 14 channels into a single 2D heatmap
    ...
    ax[1].set_title("ResNet18 Internal Heatmaps")
    ax[1].axis("off")

    plt.tight_layout()
    plt.savefig(output_path, bbox_inches='tight')
    print(f"Diagnostic heatmap saved to: {output_path}")
    plt.show()
