...
A persistent challenge for machine learning in vertebrate paleobiology remains the inherently small sample size, dictated by the scarcity of complete fossil specimens. Nevertheless, recent studies published in \textit{Paleobiology} demonstrate that by utilizing transfer learning and pre-trained models, neural networks can achieve accuracies approaching 99\%, even when constrained by datasets of merely 200 to 500 images per class \cite{AI_Paleontology}. In \textit{OsteoGen}, this limitation is managed by utilizing an experimental pipeline that matches the geometric proportions of the target's segments (head, torso, forelimb, hindlimb, and tail) against the 65 animals in the database to find the most suitable texture donors[cite: 1].
...
\section{Method}
\label{sec:method}
The approach to drafting the project, and the models trained entirely locally for the experimental results, changed three times: from a U-Net taught to extract skeletal features directly, to a prompt-conditioned ControlNet fine-tuned on the 65 selected species, and finally to a method that identifies poses and bone types via keypoints with a ResNet18. Each is detailed in its own subsection below.
Using three programs, skeleton and living-animal images obtained from the internet were isolated and placed on a black background, then resized to 512$\times$512 as required by the chosen models. The project was divided into three sequential parts based on the final outputs of each phase.
...
Early stopping was implemented for all three model training runs to prevent overfitting. Since ControlNet's results were initially the most promising, we tested it again on a lion (a similar feline is in the dataset), comparing an unconditional generation to one with a generic prompt; the results remained unsatisfactory.
...
Although this led to modest improvements, results remained unpresentable. Testing the T-Rex skeleton with two contrasting prompts (\textit{``unknown biological animal''} versus an explicit \textit{``Tyrannosaurus Rex dinosaur... reptilian skin''}) produced highly erratic outputs, alternating between sensible images and biologically illogical artifacts -- confirming that direct end-to-end generative AI could not reliably deduce extinct phenotypes without strict geometric constraints.
...
Trained on only 65 (later 68, see Section~\ref{sec:ai-use}) skeleton images, the keypoint detector generalizes poorly to body plans absent from that set. On three added extinct-species examples (\textit{T.\ rex}, \textit{Brachiosaurus}, \textit{Triceratops}), average confidence rose from 0.36--0.51 to 0.83--0.89 after a short fine-tuning run, with previously collapsed keypoints resolving into 13--14 distinct locations out of 14 and retrieval moving from a near-uniform, uninformative ranking ($\approx$1.5\% per candidate) to a differentiated one, with no loss of accuracy on the original species.

This gain did not generalize beyond the added examples' pose and style: a held-out \textit{Allosaurus} photograph (different pose, cluttered background) reproduced the original failure (confidence 0.38). This matches the small-sample-size challenge cited in Section~\ref{sec:intro} \cite{AI_Paleontology}: three examples help inputs resembling them, not the hundreds per class broad generalization would require.
...
\section*{Appendix}
%
Visual outputs and training loss curves for all three versions: kept here rather than in the main body since they document training diagnostics and abandoned intermediate steps (Version 0 and Version 1, Section~\ref{sec:method}) rather than the final visual results shown in Section~\ref{sec:experimental-results}.
